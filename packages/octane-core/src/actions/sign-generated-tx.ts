...
export async function signGeneratedTransaction(
  ...
  // Disable this check because wallets are editing our Compute Budget Unit price instructions
  // if (
  ...
  const hasFeePayerSignaturePlaceholder =
    transaction.signatures[0] && isEmptyUint8Array(transaction.signatures[0]);

  const hasAllOtherSignatures = transaction.signatures
    .slice(1)
    .every((sig) => !isEmptyUint8Array(sig));
