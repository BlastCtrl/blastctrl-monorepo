...
export type NotificationWindowProps = NotifyProps & {
  visible: boolean;
  onClose: () => void;
};
export type NotificationWindowWithPromisesProps<T> =
  NotifyWithPromisesProps<T> & {
    visible: boolean;
    onClose: () => void;
  };
...
export const notifyManyPromises = <T,>({
  ...
  const states: { state: PromiseState; label: string; txid: string }[] = Array(
    promises.length,
  );
  for (let i = 0; i < promises.length; i++) {
    states[i] = {
      ...
      label: promises[i].label,
      // @ts-expect-error we know this is defined
      txid: promises[i].txid,
    };
  }
  ...
  const getTitle = (states: { state: PromiseState; label: ReactNode }[]) => {
    ...
    if (states.some((state) => state.state === "rejected"))
      return "Confirmed with errors";
        ...
        notify(
          {
            ...
            description: <StateComponent states={states} />,
          },
          id,
        );
      })
