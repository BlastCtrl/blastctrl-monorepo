...
export const MediaFiles = ({ setValue, watch }: MediaFilesProps) => {
  ...
  const handleDropAnimationFile = (files: File[]) =>
    files[0] && setValue("animation_url", files[0]);
  ...
  return (
    <div className="mt-6 grid gap-x-6 sm:grid-cols-2">
      ...
      <div>
        <p className="py-2 text-left text-sm font-semibold">
          Additional file (animation)
        </p>
      ...
      {(image || animationUrl) && (
        <div className="mt-4 flex flex-col">
          <dd className="inline-flex items-center px-3 py-0.5 text-gray-600">
            ...
            <Tooltip
              content={
                <div className="space-y-3">
                  <p>
                    The <em>category</em> of an NFT is based on the MIME type of
                    the files linked within the metadata, with the{" "}
                    <code>animation_url</code> file having the highest
                    precedence.
                  </p>
                  <p>
                    The category is an arbitrary value, but it can be used as a
                    way for wallets/dapps to determine how an NFT should be
                    displayed.
                  </p>
                  <p>
                    Supported categories:{" "}
                    <code>image, video, audio, vr, html</code>
                  </p>
                  <p>
                    <a
            ...
            >
              <InformationCircleIcon className="ml-2 size-4 text-gray-400 hover:cursor-pointer hover:text-gray-500" />
            </Tooltip>
          </dd>
          <dt className="inline-flex max-w-fit items-center gap-x-1 rounded-full bg-blue-100 px-3 py-0.5 text-sm font-medium text-blue-800">
            ...
            <span className="font-mono">
              {animationUrl ? animationUrl.type : image?.type}
            </span>
          </dt>
        </div>
      )}
