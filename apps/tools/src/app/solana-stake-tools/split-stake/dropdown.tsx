...
interface DropdownProps<T> extends Omit<
  ListboxProps<typeof Fragment, T>,
  "as"
> {
...
export function Dropdown<T>({
  ...
  return (
    <Listbox {...props}>
      <ListboxButton className="inline-flex w-full items-center justify-between gap-x-2 rounded-lg border border-zinc-950/10 px-[calc(--spacing(3.5)-1px)] py-[calc(--spacing(2.5)-1px)] text-base/6 font-medium text-zinc-950 data-active:bg-zinc-950/2.5 data-hover:bg-zinc-950/2.5 sm:px-[calc(--spacing(3)-1px)] sm:py-[calc(--spacing(1.5)-1px)] sm:text-sm/6">
        <ListboxSelectedOption
          options={children}
